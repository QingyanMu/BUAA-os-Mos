...
*/
/*task2
Var1 counts the number of times the function is called (starting from 1 for the first time), and var2 returns the number of page status markers corresponding to the physical address
...
//	 return (Pte *)((pgtable) + PTX(va));//PTX ye biao suo yin (21~12)
...
pgtable = (Pte*) KADDR(PTE_ADDR(*pgdir_entryp));
 *ppte = pgtable + PTX(va);
//PTE_ADDR:ye biao xiang de wu li di zhi
	/*in ():Kernel Virtual Address of PTBase */
...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
	...
	if ((ret = pgdir_walk(pgdir, va, 1, &pgtable_entry)) < 0)
        	return -E_NO_MEM; // exception
    	/* Step 2. fill in the page table */
	/* Step 3 Insert page and increment the pp_ref */
